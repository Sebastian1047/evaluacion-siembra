...
app.patch('/api/participaciones/:id/estado',async(req,res,next)=>{try{const estadosPermitidos=['EN_LA_SEMANA','QUITADO_TEMPORALMENTE'];const estado=String(req.body?.estado||'').trim().toUpperCase();if(!estadosPermitidos.includes(estado))return res.status(400).json({error:'Estado de participación no válido'});const p=await getPool();const r=await p.request().input('id',sql.Int,req.params.id).input('estado',sql.VarChar(30),estado).query(`UPDATE dbo.ParticipacionSemanal SET Estado=@estado OUTPUT INSERTED.* WHERE IdParticipacion=@id`);if(!r.recordset[0])return res.status(404).json({error:'Participación no encontrada'});res.json(r.recordset[0]);}catch(e){next(e);}});
