...
function aplicarHistorialAnalista(){
  ...
  const start=Number(document.querySelector('#histStart')?.value||state.currentWeek),mode=document.querySelector('#histMode')?.value||'one',to=Number(document.querySelector('#histTo')?.value||start);
  let filtered=filas.filter(f=>{const w=Number(f.week);if(mode==='one')return w===start;if(mode==='back')return w<=start;return w<=Math.max(start,to)&&w>=Math.min(start,to)});
