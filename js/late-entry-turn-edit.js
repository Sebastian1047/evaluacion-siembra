...
(function(){
  function ensureTurnEvents(){
    ...
    (state.evals||[]).forEach(e=>{
      if(e.turn==null){
        ...
        let idx=same.indexOf(e);e.turn=idx>=0?idx+1:null;
      }
    });
  }
  ensureTurnEvents();
  ...
  if(typeof commitEval==='function'){
    ...
    commitEval=function(ids){
      ...
      let expected=(Number(p.sampleTurn)||0)+1,before=(state.evals||[]).length,result=commitEvalBeforeTurnNumber(ids);
      let created=(state.evals||[]).slice(before).find(e=>e.person===p.id);if(created&&created.turn==null){created.turn=expected;save()}return result;
    };
  }
  ...
  function criterionChecks(selected){
    selected=selected||[];
    return seed.criteria.map(c=>`<label><input type="checkbox" name="edit-turn-crit" value="${c[0]}" ${selected.includes(c[0])?'checked':''}> ${c[1]} ${c[2]?'<span class="badge">Crítico</span>':''}</label>`).join('');
  }
  function personTurnRows(p){
    ensureTurnEvents();let start=Math.max(1,Number(p.entryTurn)||1),end=Number(p.sampleTurn)||0;if(end<start)return '<div class="card muted">Todavía no hay turnos resueltos para esta persona.</div>';
    let rows=[];
    for(let turn=end;turn>=start;turn--){
      let omission=omissionAt(p,turn),evaluation=evaluationAt(p,turn),failures=evaluation&&Array.isArray(evaluation.failures)?evaluation.failures:[];
  ...
  window.showPreviousTurns=function(){
    ...
    modal(`<h3>Corregir turno anterior</h3><p><b>${p.name}</b></p><p class="muted">Seleccione un turno para corregir si hubo evaluación, si no se realizó muestra y, cuando exista evaluación, los ítems que incumplieron.</p><div style="max-height:360px;overflow:auto">${personTurnRows(p)}</div><button class="btn ghost block" onclick="closeModal()">Cerrar</button>`);
  };
  ...
  window.savePreviousTurnCorrection=function(turn){
    ...
    ensureTurnEvents();
    let desired=document.querySelector('#edit-turn-state')?.value||'evaluation';
    let failures=[...document.querySelectorAll('[name=edit-turn-crit]:checked')].map(x=>x.value);
    let evaluation=evaluationAt(p,turn),omission=omissionAt(p,turn),wasEval=!omission;
    ...
    if(desired==='evaluation'){
      ...
      evaluation.failures=failures;evaluation.score=Math.max(0,100-failures.length*8);evaluation.corrected=true;evaluation.correctedAt=new Date().toISOString();
      if(!wasEval){p.done=(Number(p.done)||0)+1;p.required=(Number(p.required)||0)+1;state.pending=(Number(state.pending)||0)+1}
    }else{
      if(evaluation)state.evals=(state.evals||[]).filter(e=>e!==evaluation);
      if(!omission)state.sampleTurnOmissions.push({week:state.currentWeek,person:p.id,turn:Number(turn),date:new Date().toISOString(),corrected:true});
      if(wasEval){p.done=Math.max(0,(Number(p.done)||0)-1);p.required=Math.max(p.done,(Number(p.required)||30)-1)}
    }
    save();
    let result=desired==='evaluation'?(failures.length?`Evaluación realizada · ${failures.length} ítem${failures.length===1?'':'s'} con incumplimiento`:'Evaluación realizada · todos los ítems cumplen'):'No realizar muestra';
    modal(`<h3>Turno ${turn} corregido</h3><p><b>${result}</b></p><p class="muted">La persona continúa en su turno operativo actual.</p><button class="btn primary block" onclick="closeModal();render()">Aceptar</button>`);
  };

  function decoratePreviousTurnButton(){
    if(state.role!=='monitor'||state.view!=='seguimiento')return;let p=current();if(!p||(Number(p.sampleTurn)||0)<(Number(p.entryTurn)||1))return;
