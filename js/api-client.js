...
(function () {
  ...
  async function loadCriteriaIntoSeed(){if(typeof seed==='undefined'||!Array.isArray(seed.criteria))throw new Error('seed.criteria no está disponible');const localFallback=seed.criteria.map(c=>[...c]);try{const items=await getItems();const activeItems=Array.isArray(items)?items.filter(item=>item.activo!==false):[];if(!activeItems.length)throw new Error('La API no devolvió ítems activos');window.SiembraAzureItemIds=Object.fromEntries(activeItems.map(item=>[String(item.codigo??item.Codigo),Number(item.id??item.IdItem)]));seed.criteria=activeItems.map(item=>[item.codigo??item.Codigo,item.nombre??item.Nombre,Boolean(item.esCritico??item.EsCritico)]);window.SiembraCriteriaSource='azure';return seed.criteria;}catch(error){seed.criteria=localFallback;window.SiembraAzureItemIds={};window.SiembraCriteriaSource='local-fallback';console.warn('No fue posible cargar los ítems desde Azure; se usan los criterios locales.',error);return seed.criteria;}}
